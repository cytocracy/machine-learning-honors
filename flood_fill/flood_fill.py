...
def flood_fill(image, start, newColor):
    ...
    q = [start]
    initcolor = getPixel(image, start[0], start[1])

    visited = set()
    while len(q) > 0:
        currentLocation = q.pop()
        row, col = currentLocation
        if currentLocation not in visited:
            visited.add(currentLocation)
            setPixel(image, row, col, newColor)
            # time.sleep(.01)
            neighbors = getNeighbors(image, currentLocation)
            for n in neighbors:
                if getPixel(image, n[0], n[1]) == initcolor:
                    q.append(n)


def getNeighbors(image, location):
    row, col = location
    neighbors = []
    if row != 0:
